...
//    Coupon Data
    public static function CouponsData($data)
    {
        ...
        if (isset($data['graph']) && isset($data['type'])) {
            $coupons = coupon::where('status', 1)->where('country_id', $coun)->whereJsonContains('coupon_graph', $data['graph'])->where('coupon_type',$data['type'])->orderBy('id', 'DESC')->get();
        }
            elseif (isset($data['graph'])) {
                ...
                if($collection){
                $coupons = coupon::where('status', 1)->where('country_id', $coun)->where('category_id',$collection->id)->get();
                }else{
                   return false;
                ...
                $coupons = coupon::where('status', 1)->where('country_id', $coun)->where('store_id',$store->id)->get();

            }
            elseif(isset($data['coupon_type'])){
                $coupons = coupon::where('status', 1)->where('country_id', $coun)->where('coupon_type',$data['coupon_type'])->get();
            }
            return $coupons;
